...
<div className=" bg-white shadow p-4 py-8">
        ...
        <div className="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
            ...
            <div onClick={addition} className="buttons flex justify-end">
    ...
    <div className={`item clicker px-6 py-6 `} x-data="{isOpen : false}" onClick={open}>
      {blogtitle.map((title)=><a className="flex items-center justify-between no-underline" >
                    <h4 >{title}</h4>
                    <svg 
              ...
              {blogpara.map((para)=><div x-show="isOpen"  className={`mt-3 content ${show}`}>
                    <p>{para}</p>
                </div>)}
